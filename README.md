...
- **Double notation** : IPA (Allosaurus) + ALF Rousselot-Gillieron
- **Base ALF locale** (Atlas Linguistique de la France, scrape SYMILA Toulouse) :
  - 639 points d'enquete geolocalises (commune, departement, dialecte)
  - ~525 cartes de concepts
  - 33 217 phrases historiques (1902-1910)
- Saisie du **point d'enquete** par carte ou par commune
- Comparaison automatique des attestations historiques vs la transcription en cours
- Pipeline avec nettoyage des noms propres et normalisation
...
Clipr/
  ...
  data/
    alf.db                   # Base ALF SQLite (scrape SYMILA, ~250 Mo)
  scripts/
    transcribe.py            # faster-whisper
    alf-scrape.py            # Aspirateur SYMILA -> data/alf.db
  docs/
    index.html
...
| Phonetique | Allosaurus + ALF Rousselot | IPA universel + notation historique francaise |
| Donnees ALF | SYMILA Toulouse (scrape) | Atlas linguistique de reference |
| LLM | Ollama HTTP | API simple, gere les modeles tout seul |
| Temps reel | ws (WebSocket) | Push de progression au front |
| Deploiement | Docker Compose + Caddy | HTTPS automatique, multi-services |
